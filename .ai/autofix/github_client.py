...
class GitHubClient:
    ...
    def _run_gh(self, args: List[str]) -> Tuple[int, str]:
        ...
        try:
            ...
            self.last_error = res.stderr.strip() if res.returncode != 0 else ""
    ...
    def fetch_open_ai_tasks(self) -> List[Dict[str, Any]]:
        code, out = self._run_gh([
            "issue", "list", "--label", "ai-task", "--state", "open",
    ...
    def fetch_active_prs(self) -> List[Dict[str, Any]]:
        code, out = self._run_gh([
            "pr", "list", "--state", "open",
            "--json", "number,title,headRefName,baseRefName,commits"
        ])
        if code == 0 and out:
            try:
                return json.loads(out)
    ...
    def create_or_update_issue(
        ...
        body: str,
        labels: List[str],
        existing_issue_num: Optional[int] = None
    ) -> int:
    ...
    def _ensure_label(self, label: str):
        # Creating an existing label fails harmlessly; the following issue edit still proceeds.
        self._run_gh(["label", "create", label, "--color", "ededed"])
    ...
    def update_issue_labels(
        ...
        for lbl in add_labels:
            ...
            code, _ = self._run_gh(["issue", "edit", str(issue_num), "--add-label", lbl])
            ok = ok and code == 0
        for lbl in remove_labels:
            code, _ = self._run_gh(["issue", "edit", str(issue_num), "--remove-label", lbl])
            if code != 0 and "not found" not in self.last_error.lower():
                ok = False
        return ok
