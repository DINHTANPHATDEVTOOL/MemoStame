...
-- Step 2.1: Drop dependent RLS policies referencing feed post and comment IDs before type alteration
DROP POLICY IF EXISTS "Select feed reactions if post visible" ON public.feed_reactions;
DROP POLICY IF EXISTS "Select feed comments if post visible" ON public.feed_comments;

-- Step 2.2: Safely drop foreign key constraints referencing feed content IDs before type conversion
...
-- Step 2.3: Drop default UUID generators on content ID columns if any
...
-- Step 2.4: Standardize content identifier columns as TEXT for full compatibility with client IDs (e.g., feed_post_1)
...
-- Step 2.5: Re-create Foreign Keys referencing TEXT content IDs
...
-- Step 2.6: Re-verify unique reaction constraint (post_id, user_id)
...
-- Step 2.7: Re-create RLS policies with direct TEXT equality
